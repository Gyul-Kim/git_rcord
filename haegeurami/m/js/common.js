...
// header
$('header .btn .open').css('display', 'block');
$('header .btn .close').css('display', 'none');

$('header .btn').on('click', function(){

	if($("header nav").is(".open") === true){

		$("header nav").removeClass("open");
		$("header nav").css({"left":"-100%","opacity":"0"});
		...
		$('.gnb li, .lnb').removeClass('active');
		$('header .lnb .lnb').css({'display':'none'});
	
		$("header nav").addClass("open");
		$("header nav").css({"left":"0","opacity":"1"});

		$('header .btn .close').css('display', 'block');
		$('header .btn .open').css('display', 'none');
...
$('header .gnb li').on('click', function() {
	...
	if($(this).is('.haveLnb')){
		...
		$(this).siblings().find('.lnb').stop().slideDown();
...
switch(path) {
    ...
    case 'index' :
		...
		for(var i = 0; i < img[0]; i++){
			$(".visual .Swipers .swiper-image").append(
				'<div class="swiper-slide">' + 
					'<div style="background-image:url(' + url +'m/main/'+  (i + 1) + '.jpg)"></div>' + 
				'</div>'
			);
		...
		function(data){
			for(var i = 0; i < img[2].length; i++) {
				$(".room_swiper .swiper2 .swiper-image").append(
					'<div class="swiper-slide">' + 
						'<div style="background-image:url(' + url + 'm/room/' + (i + 1) + '/1.jpg)"></div>' + 
		...
		$(window).on("scroll", function() {
			...
			var start1 = $(".section").eq(2).find("h3").offset().top - $(".section").height();
			var start2 = $(".section").eq(3).find("h3").offset().top - $(".section").height();
			var start3 = $(".section").eq(4).find("h3").offset().top - $(".section").height();
			var start4 = $(".section").eq(5).find("h3").offset().top - $(".section").height();
			
			var start5 = $("#index .info .movieWrap").offset().top - $(".section").height();
	...
	case 'about' :
		$.getJSON('http://digitalnow.co.kr/reserve/pensionInfo/'+ account +'/4',//사업자 정보
			...
			for(var i = 0; i < img[1]; i++){
				$(".visual .Swipers .swiper-image").append(
					'<div class="swiper-slide">' + 
						'<div style="background-image:url(' + url + 'm/exterior/' + (i + 1) + '.jpg)"></div>' + 
					'</div>'
				);
			} Swipers(".visual", "auto", true, 0, false);
		...
		function(data){
			...
			$(".gallery .gallery_top .gallery_right").append(
				'<div class="circle_wrap">' +
					'<div style="background:url(https://gonylab8.speedgabia.com/heagrami/m/room/' + (detailPath + 1)  +'/1.jpg) no-repeat 50% 50%; background-size:cover;"></div>' +
				'</div>'
			);
			...
			for(var i = 0; i < arr4.length; i++){
				for(var e = arr4[i]; e < arr5[i]+1; e++){
					$(".menu .Swipers .swiper-image").eq(i).append(
						'<div class="swiper-slide">'+ 
								'<a href="rooms.html?num='+ numbering(e) + '"><div style="background-image:url(http://gonylab8.speedgabia.com/heagrami/m/room/'+ (e + 1) + '/1.jpg)"></div></a>' + 
	...
	case 'camping' :
		...
		for(var i = 0; i < img[3][1]; i++){
			$(".Slides .slide").append('<li id="i'+ numbering(i) +'" style="background-image:url('+ url +'m/camping/'+ '/' + (i + 1) +'.jpg)"></li>');
		...
		for(var i = 0; i < img[3][0]; i++){
			$(".visual .Swipers .swiper-image").append(
				'<div class="swiper-slide">' + 
					'<div style="background-image:url(' + url + 'm/camping/' +  (i + 1) + '.jpg)"></div>' + 
				'</div>'
			);
		} Swipers(".visual", "auto", true, 0, false);
		...
		for(var i = 0; i < img[3][1]; i++){
			$(".facility .camp_swiper .swiper-image").append(
				'<div class="swiper-slide">' + 
					'<div style="background-image:url(' + url + 'm/camping_facility/' + (i + 1) + '.jpg)"></div>' + 
				'</div>'
			);
	...
	case 'cafe' :
		...
		for(var i = 0; i < img[4]; i++){
			$(".Slides .slide").append('<li id="i'+ numbering(i) +'" style="background-image:url('+ url +'m/cafe/'+ '/' + (i + 1) +'.jpg)"></li>');
		} Slides("Slides","slide > li");	
		...
		for(var i = 0; i < img[4]; i++){
			$(".visual .Swipers .swiper-image").append(
				'<div class="swiper-slide">' + 
					'<div style="background-image:url(' + url + 'm/cafe/' + (i + 1) + '.jpg)"></div>' + 
				'</div>'
			);
		} Swipers(".visual", "auto", true, 0, false);
